    ...
    def extract_text_smart(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if file_ext == ".docx" and visual_analysis["has_visuals"]:
                ...
                if visual_analysis["has_visuals"] and self.batch_processor and file_ext == ".pdf":
                    extraction = self._extract_with_batch_vlm(file_path)
                else:
                    extraction = self._extract_standard(file_path)
            # 🚩 PATCH END
            ...
            result.update(extraction)
            print("Extracted text from extractor:", result.get("text", "")[:500])
            # Patch to ensure accurate page count for converted PDFs
            if "pages" not in result or not result["pages"]:
                # 📄 Correct page count for converted PDFs
                if file_ext == ".pdf" or (file_ext == ".docx" and result.get("extraction_method") == "docx->pdf+hybrid_vlm_text"):
                    result["pages"] = get_pdf_page_count(pdf_path if result.get("extraction_method") == "docx->pdf+hybrid_vlm_text" else file_path)
                elif file_ext == ".docx":
                    result["pages"] = get_docx_page_estimate(result.get("text", ""))
                elif file_ext == ".txt":
                    lines = result.get("text", "").count('\n')
